...
class Adversarial(nn.Module):
    def __init__(self, args, gan_type='WGAN_GP', build_optimizer=None):
        super(Adversarial, self).__init__()
        self.gan_type = gan_type
        self.gan_k = args.gan_k
        self.dis = self.build_discriminator(args)
        if gan_type == 'WGAN_GP':
        ...
        self.optimizer = build_optimizer(optim_args, self.dis)

    def build_discriminator(self, args):
        dis = None
        if args.discriminator_type == 'NLayerDiscriminator':
            dis = discriminator.Discriminator(args)
        elif args.discriminator_type == 'Discriminator_VGG':
            dis = discriminator.Discriminator_VGG(args)
        else:
            raise  NotImplementedError('Discriminator model [{:s}] not recognized'.format(args.discriminator_type))
        return dis
        ...
        for _ in range(self.gan_k):
            ...
            if self.gan_type == 'GAN':
                ...
                if self.gan_type.find('GP') >= 0:
                    epsilon = torch.rand_like(fake).reshape(-1, 1, 1, 1)
