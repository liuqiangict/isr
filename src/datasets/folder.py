...
def make_dataset_hr(dir, extensions=None, is_valid_file=None):
    images = []
    dir = os.path.expanduser(dir)
    if not ((extensions is None) ^ (is_valid_file is None)):
        raise ValueError("Both extensions and is_valid_file cannot be None or not None at the same time")
...
class HRDatasetFolder(VisionDataset):
    def __init__(self, root, scale, patch_size=0, transform=None, 
        loader=imageio_loader, is_valid_file=None, is_same_size=False):
        super(HRDatasetFolder, self).__init__(root, transform=transform)
        samples = make_dataset_hr(self.root, IMG_EXTENSIONS if is_valid_file is None else None, is_valid_file)
        if len(samples) == 0:
            raise (RuntimeError("Found 0 files in subfolders of: " + self.root + "\n"
                                "Supported extensions are: " + ",".join(extensions)))
    ...
    def __getitem__(self, index):
        ...
        if self.patch_size < 0:
            ...
            return hr, os.path.splitext(os.path.basename(path))[0]

        h, w = hr_img.shape[:2]
        rescale_h = h // self.scale
        rescale_w = w // self.scale
        hr = hr_img[:rescale_h * self.scale, :rescale_w * self.scale, :]
        lr = imresize(hr, output_shape=(rescale_h, rescale_w)) 
        if self.is_same_size:
            lr = imresize(hr, output_shape=(rescale_h * self.scale, rescale_w * self.scale))
        

        if self.patch_size > 0:
            lr, hr = get_patch(lr, hr, self.patch_size, self.scale, self.is_same_size)

        if self.transform is not None:
            hr = self.transform(hr)
            lr = self.transform(lr)

        return hr, lr, os.path.splitext(os.path.basename(path))[0]
